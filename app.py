...
if f'Attendance-{datetoday}.csv' not in os.listdir('Attendance'):
    with open(os.path.join('Attendance', f'Attendance-{datetoday}.csv'), 'w') as f:
...
def identify_face(facearray):
    model = joblib.load(os.path.join('static', 'face_recognition_model.pkl'))
...
def train_model():
    ...
    for user in userlist:
        for imgname in os.listdir(os.path.join('static/faces', user)):
            img = cv2.imread(os.path.join('static/faces', user, imgname))
    ...
    joblib.dump(knn, os.path.join('static', 'face_recognition_model.pkl'))
...
def add_attendance(name):
    ...
    current_time = datetime.now().strftime("%I:%M:%S %p")  # Change the time format here
